...
function Slider({
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {/* Omit accent- and h- classes — globals.css handles slider styling */}
      <input
        type="range"
        min={min}
        max={max}
  ...
  return (
    <div
      className={`flex flex-col gap-1 rounded-xl p-4 ${
        highlight
          ? "bg-[#00AEEF]/[0.06] border border-[#00AEEF]/20"
          : "bg-white dark:bg-[#1A1A1A] border border-[#E5E7EB] dark:border-white/[0.08]"
      }`}
    >
      <p className="text-xs font-semibold text-gray-500 dark:text-gray-400">{label}</p>
      <p
        className={`text-2xl font-extrabold ${highlight ? "text-[#00AEEF]" : "text-[#0A0A0A] dark:text-[#F0F0F0]"}`}
...
export default function ROICalculatorContent() {
  ...
  const isBigWin = annualSavings >= 100_000;
      ...
      <section className="relative w-full border-b border-[#E5E7EB] dark:border-white/[0.06] bg-hero-gradient">
        <div className="mx-auto max-w-7xl px-6 py-16 lg:px-8 lg:py-20 text-center">
          ...
          <p className="mt-5 mx-auto max-w-2xl text-lg leading-8 text-gray-600 dark:text-gray-400">
            See how much time and money EZee Assist can save your franchise network.
      ...
      <section className="w-full bg-[#F7F8FA] dark:bg-[#111111]">
        <div className="mx-auto max-w-7xl px-6 py-16 lg:px-8">
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-2">
            ...
            >
              <h2
                className="text-lg font-bold text-[#0A0A0A] dark:text-[#F0F0F0]"
              ...
              <Slider
                ...
                value={locations} min={10} max={1000}
                onChange={setLocations}
              />
              <Slider
                label="Support questions per location per week"
                value={questionsPerLocation} min={5} max={50}
                onChange={setQuestionsPerLocation}
              />
              <Slider
                label="Average time to answer one question (minutes)"
                value={minutesPerQuestion} min={5} max={30}
                onChange={setMinutesPerQuestion}
              />

              <div className="flex flex-col gap-2">
                <label className="text-sm font-semibold text-[#0A0A0A] dark:text-[#F0F0F0]">
                  Average hourly cost of support staff
                </label>
            ...
            >
              <div className="rounded-2xl border border-[#E5E7EB] dark:border-white/[0.08] bg-white dark:bg-[#161616] p-8 shadow-[0_1px_3px_rgba(0,0,0,0.04)] dark:shadow-[0_1px_3px_rgba(0,0,0,0.3)]">
                <h2
                  className="text-lg font-bold text-[#0A0A0A] dark:text-[#F0F0F0] mb-6"
                  style={{ letterSpacing: "-0.01em" }}
                >
                  Your results
                </h2>
                ...
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <StatCard label="Total weekly questions"  value={weeklyQuestions.toLocaleString()} />
                  <StatCard label="Weekly hours on support" value={weeklyHours.toFixed(1) + "h"} />
                </div>
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <StatCard label="Annual support cost"     value={fmt(annualCost)} />
                  <StatCard label="Questions deflected/week" value={Math.round(deflected).toLocaleString()} />
                </div>
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <StatCard label="Annual hours saved"      value={Math.round(annualHoursSaved).toLocaleString() + "h"} />
                  <StatCard label="Estimated payback"       value="< 3 months" />
                </div>
                ...
                <div className="mt-4 rounded-2xl border border-[#00AEEF]/30 bg-[#00AEEF]/[0.05] p-6 text-center">
                  ...
                  <div className="relative inline-block">
                    {isBigWin && (
                      <Sparkles
                        size={18}
                        className="absolute -top-3 -right-4 text-[#00AEEF] animate-sparkle"
                        strokeWidth={1.75}
                      />
                    )}
                    <motion.p
                      ...
                      className={`text-5xl font-extrabold text-[#00AEEF] savings-glow ${
                        isBigWin ? "animate-sparkle" : ""
                      }`}
          ...
          <div className="mt-12 rounded-2xl border border-[#E5E7EB] dark:border-white/[0.08] bg-white dark:bg-[#161616] p-8 text-center shadow-[0_1px_3px_rgba(0,0,0,0.04)] dark:shadow-[0_1px_3px_rgba(0,0,0,0.3)]">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6">
              Our team will build a custom ROI model based on your actual support data.
            </p>
            <div className="flex flex-wrap justify-center gap-4">
              ...
              <button onClick={() => setShow(true)}>
                <Button size="lg" variant="secondary">
                  Or download the full ROI breakdown
                </Button>
              </button>
            </div>
          </div>
