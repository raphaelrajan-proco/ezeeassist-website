        ...
        <div className="mx-auto max-w-7xl px-6 py-24 lg:px-8 lg:py-28">
          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
            {painPoints.map(({ icon: Icon, title, body }, i) => (
              <motion.div key={title} {...fadeUp(i * 0.1)} className="card-hover group relative overflow-hidden rounded-2xl border border-[#E5E7EB] bg-white p-8 shadow-[0_1px_3px_rgba(0,0,0,0.04),_0_4px_16px_rgba(0,0,0,0.04)]">
      ...
      <section style={{ background: "linear-gradient(to bottom, #F7F8FA 0%, #ffffff 100%)" }} className="w-full">
        <div className="mx-auto max-w-7xl px-6 py-24 lg:px-8 lg:py-28">
          <motion.div className="text-center mb-16" {...fadeUp(0)}>
            ...
            <p className="mt-4 text-base text-gray-500 max-w-lg mx-auto font-medium">No setup on your end. No new tools for your team to learn.</p>
          </motion.div>
          <div className="relative grid grid-cols-1 gap-10 md:grid-cols-3">
            ...
            {howItWorks.map(({ icon: Icon, step, title, sub }, i) => (
              <motion.div key={step} {...fadeUp(i * 0.12)} className="relative flex flex-col items-center text-center md:items-start md:text-left">
                ...
                <h3 className="text-base font-bold text-[#0A0A0A] mb-2" style={{ letterSpacing: "-0.01em" }}>{title}</h3>
                <p className="text-sm leading-6 text-gray-600">{sub}</p>
              </motion.div>
            ))}
          </div>
        </div>
