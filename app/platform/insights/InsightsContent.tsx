          ...
          <div className="grid grid-cols-1 gap-8 sm:grid-cols-2">
            {capabilities.map(({ icon: Icon, title, body }, i) => (
              <motion.div
                key={title}
                {...fadeUp(i * 0.08)}
                className="card-hover group relative overflow-hidden rounded-2xl border border-[#E5E7EB] bg-white p-8 shadow-[0_1px_3px_rgba(0,0,0,0.04),_0_4px_16px_rgba(0,0,0,0.04)]"
              >
                  ...
                  <div className="w-full space-y-2">
                    {[85, 62, 54, 41, 38].map((w, i) => (
                      <div key={i} className="flex items-center gap-3">
                        <div className="w-28 truncate text-right text-[9px] text-gray-300">
                          {["Brand guidelines","Vendor approvals","Marketing SOPs","Onboarding docs","HR policies"][i]}
                        </div>
                        <div className="flex-1 rounded-full bg-[#E5E7EB] h-2.5">
                          <div
                            className="h-2.5 rounded-full bg-[#00AEEF]/60"
                            style={{ width: `${w}%` }}
                          />
                        </div>
                        <span className="text-[9px] font-semibold text-gray-400">{w}</span>
                      </div>
                    ))}
                  </div>
