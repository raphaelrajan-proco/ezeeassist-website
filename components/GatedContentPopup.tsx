    ...
    <AnimatePresence>
      {show && (
        <>
          ...
          >
            <div className="relative w-full max-w-3xl rounded-2xl bg-white dark:bg-[#161616] shadow-[0_24px_80px_rgba(0,0,0,0.2)] dark:shadow-[0_24px_80px_rgba(0,0,0,0.6)] overflow-hidden max-h-[90vh] overflow-y-auto">
              {/* Close button */}
              <button
                onClick={handleClose}
                className="absolute top-4 right-4 z-10 flex h-8 w-8 items-center justify-center rounded-full border border-[#E5E7EB] dark:border-white/[0.08] bg-white dark:bg-[#1A1A1A] text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition-colors"
                aria-label="Close"
              >
                <X size={15} />
              </button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2">
                {/* Left — offer */}
                <div className="flex flex-col justify-center p-8 md:p-10 bg-gated-popup-gradient">
                  {/* Report cover placeholder */}
                  <div className="flex items-center justify-center w-full aspect-[3/4] max-w-[200px] mx-auto rounded-xl border border-[#E5E7EB] dark:border-white/[0.08] bg-gradient-to-br from-[#00AEEF]/10 to-[#00AEEF]/5 shadow-md mb-6">
                ...
                {/* Right — form */}
                <div className="p-8 md:p-10 border-t md:border-t-0 md:border-l border-[#E5E7EB] dark:border-white/[0.08]">
                  {submitted ? (
                    <div className="flex flex-col items-center justify-center h-full text-center gap-4 py-8">
                      <div className="flex h-14 w-14 items-center justify-center rounded-full bg-[#00AEEF]/10">
                        <span className="text-2xl">📬</span>
                      </div>
                      <h3 className="text-lg font-bold text-[#0A0A0A] dark:text-[#F0F0F0]">You&apos;re all set!</h3>
                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="block text-xs font-semibold text-[#0A0A0A] dark:text-[#F0F0F0] mb-1">First Name *</label>
                          <input name="firstName" type="text" required placeholder="Jane" className={inputClass} />
                        </div>
                        <div>
                          <label className="block text-xs font-semibold text-[#0A0A0A] dark:text-[#F0F0F0] mb-1">Last Name *</label>
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-[#0A0A0A] dark:text-[#F0F0F0] mb-1">Company *</label>
                        <input name="company" type="text" required placeholder="Franchise Brand Inc." className={inputClass} />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-[#0A0A0A] dark:text-[#F0F0F0] mb-1">Number of Locations</label>
