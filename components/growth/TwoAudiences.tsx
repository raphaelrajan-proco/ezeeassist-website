...
function HqMockup() {
  return (
    <GradientFrame>
      <div className="rounded-xl overflow-hidden mx-auto max-w-[300px]" style={MOCK_SURFACE}>
        <div className="flex items-center justify-between px-3 py-2" style={{ borderBottom: `1px solid ${MOCK_HAIRLINE}` }}>
          <span className="text-[10px]" style={{ color: MOCK_TEXT, fontWeight: 600 }}>Policies</span>
          <span className="text-[8.5px] rounded-full px-1.5 py-0.5" style={{ backgroundColor: "rgba(22,163,74,0.12)", color: "#15803D", fontWeight: 700 }}>
        ...
        <div className="p-2.5 space-y-1.5">
          {POLICIES.map((p) => (
            ...
            >
              <span className="text-[10px]" style={{ color: MOCK_TEXT, lineHeight: 1.3 }}>{p.label}</span>
              <span
                aria-hidden="true"
    ...
    <GradientFrame>
      <div className="rounded-xl overflow-hidden mx-auto max-w-[300px]" style={MOCK_SURFACE}>
        <div className="px-3 py-2" style={{ borderBottom: `1px solid ${MOCK_HAIRLINE}` }}>
          <span className="text-[10px]" style={{ color: MOCK_TEXT, fontWeight: 600 }}>Store #214</span>
        </div>
        <div className="p-2.5">
          {/* Prompt box */}
          <div
          ...
          >
            <span className="text-[10px]" style={{ color: MOCK_MUTED }}>Ask, run, or build something</span>
          </div>

          <p className="text-[8.5px] uppercase tracking-[0.12em] mb-1.5" style={{ color: MOCK_MUTED, fontWeight: 600 }}>
            Saved tools
          </p>
          <div className="space-y-1.5">
            {[
              ...
              >
                <p className="text-[10px]" style={{ color: MOCK_TEXT, fontWeight: 600 }}>{t.name}</p>
                <p className="text-[8.5px] mt-0.5" style={{ color: t.mine ? "#0077A8" : MOCK_MUTED, fontWeight: 500 }}>
                  {t.by}
                </p>
              </div>
            ))}
