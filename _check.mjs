...
const s=await p.evaluate(()=>{
  ...
  return {secs:[...document.querySelectorAll("main section")].map(x=>`${x.id||"-"}:${getComputedStyle(x).backgroundColor}`),
    h1:document.querySelectorAll("main h1").length,h2:document.querySelectorAll("main h2").length,
    em:(t.match(/—/g)||[]).length,
    ...
    illustrative:(t.match(/Illustrative/gi)||[]).length,
    tbd:(t.match(/\{\{TBD/g)||[]).length,
    trustCenter:t.includes("/trust-center"),
...
console.log(`h1=${s.h1} h2=${s.h2} em=${s.em} banned=${s.banned.length?s.banned.join(","):"none"}`);
console.log(`"Illustrative" labels=${s.illustrative} | {{TBD}}=${s.tbd} | #00AEEF text=${s.accent00}`);
await b.close();
